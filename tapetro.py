...
def get_fuel_price(data: dict, brand_name: str) -> dict:
    ...
    try:
        for comp_dict in data:
            office_id = comp_dict['TASiteNumber']
            prices = comp_dict['FuelPrices']
            fuel_prices_dict.update({office_id: {}})
            for item in prices:
                fuel_code = item['FuelCode']
                if fuel_code == 20:
                    fuel_iter_dict['diesel'] = item.get('Price', '').replace('$', '')
                elif fuel_code == 143:
                    fuel_iter_dict['def'] = item.get('Price', '').replace('$', '')
                fuel_prices_dict[office_id].update(fuel_iter_dict)
...
@timed
def result_tapetro(db_data: dict, brand_name: str) -> dict:
    prices_dict = {}
