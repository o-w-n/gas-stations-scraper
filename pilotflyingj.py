...
def get_response(brand_name: str) -> dict:
    response = ''
    headers = {
    ...
    try:
        url = "https://pilotflyingj.com/umbraco/surface/FuelPrices/UpdatePrices"
        params = {
...
def get_fuel_price(data: dict, brand_name: str) -> dict:
    fuel_prices_dict = {}
    try:
        for comp_dict in data['Prices']:
            office_id = comp_dict['StoreNumber']
            fuel_prices_dict.update({office_id: {}})
            fuel_prices_dict[office_id]['diesel'] = comp_dict.get('Diesel').replace('$', '')
            fuel_prices_dict[office_id]['def'] = comp_dict.get('PumpDef').replace('$', '')
...
@timed
def result_pilotflyingj(db_data: dict, brand_name: str) -> dict:
    prices_dict = {}
    fuel_price = get_fuel_price(get_response(brand_name), brand_name)
